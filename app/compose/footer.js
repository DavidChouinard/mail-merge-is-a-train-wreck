...
export default React.createClass({
  ...
  render: function() {
    var button_class = "email-actions-send";
    if (this.props.compose.state.mail_merge_mode && (this.props.compose.state.merge_data === null || this.props.compose.state.merge_data.length <= 1)) {
      button_class += " disabled";
    }

    return <div className="email-actions email-shell">
        <div className={button_class} onClick={this.send_emails}>
          Send{this.props.compose.state.mail_merge_mode && this.props.compose.state.merge_data !== null ? ' ' + (this.props.compose.state.merge_data.length - 1) + ' emails' : ''}
        </div>
        <div className="email-actions-mass" onClick={this.props.compose.toggle_mail_merge_mode}>
